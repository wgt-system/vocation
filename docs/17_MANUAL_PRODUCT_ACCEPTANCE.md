...
After the blocking product work is implemented, the manual current-market procedure in `docs/18_FIRST_USER_ACCEPTANCE.md` must be repeated against real data. Only then is the next release version chosen and the exact release candidate promoted.
