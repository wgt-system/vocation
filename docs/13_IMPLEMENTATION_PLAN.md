...
## Slice 15 – Application Case and private Application Material (Semantik eingefroren, Implementierung folgt)

Vocation besitzt ApplicationCase-Fachsemantik als Aggregate pro Opportunity, getrennt vom Opportunity Tracking Status. V1 definiert den expliziten Lifecycle `draft`, `ready`, `submitted`, `interviewing`, `offer`, `accepted`, `rejected`, `withdrawn`, historische Lifecycle Events, höchstens einen aktiven/nonterminal Case je Opportunity und private ApplicationMaterial-Metadaten mit Arten `cv`, `cover_letter`, `other`. Research/Availability, Groups/Waves, automatische Submission, E-Mail/Calendar-Übergänge, Dokument-Rendering, Storage, Encryption, WGT/Conveyance und Cross-device Writes bleiben ausgeschlossen.
